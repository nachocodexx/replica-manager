...
//    _ <- finalizer
  ...
  def run(ars:List[NodeX],events:List[EventX], hotData:List[DumbObject])(implicit ctx:NodeContext) = {
    for {
      currentState   <- ctx.state.get
      maybeBalancer  = currentState.uploadBalancer
      _              <- maybeBalancer match {
        case Some(lb) =>  for {
          _                     <- IO.unit
          numberOfNodes         = ars.length
          arMap                 = ars.map(node => node.nodeId -> node).toMap
          //          NODES WITH CACHE_SIZE GREATER THAN 0
          freeCacheSizeNodes   = ars.filter(_.availableCacheSize>0)
          freeCacheSizeNodeIds = freeCacheSizeNodes.map(_.nodeId)
          //
          distributionSchema = Events.generateDistributionSchema(events=events)
...
//            .filter(_._2.length < currentState.maxRF)
          ...
          availableNodesToReplicateMap = filteredDistributionSchema.map{
            case (objectId, replicas) =>
              objectId -> freeCacheSizeNodeIds.toSet.diff(replicas.toSet).toList
          }.map{
          ...
          _ <- ctx.logger.debug(s"AVAILABLE_NODES_TO_REPLICATE ${availableNodesToReplicateMap.map(x=>x._1->x._2.length)}")
          //            _______________________________________________________________________________________
          startedAtSelectedNode <- IO.monotonic.map(_.toNanos)
          _ <- availableNodesToReplicateMap.toList.traverse{
            case(objectId,nodeCandidates)=>
              val relativeHotData = hotData.find(_.objectId == objectId)
              val maybeObjectSize = relativeHotData.map(_.objectSize)
              // SELECT A NODE
              val maybeSelectedNodeAndObjectSize =  for {
                objectSize   <- maybeObjectSize
                selectedNode <- lb.balance(objectSize= objectSize, nodes = nodeCandidates)
              } yield (selectedNode,objectSize)

//              for {
                maybeSelectedNodeAndObjectSize match {
                  case Some((selectedNode,replicateObjectSize)) =>  for {
                    _              <- IO.unit
                    endedAtSelectedNode <- IO.monotonic.map(_.toNanos)
                    selectedServiceTime  = endedAtSelectedNode - startedAtSelectedNode
//                    _                  <- ctx.logger.info(s"UPLOAD $objectId $selectedNodeId $serviceTimeNanos $operationId")
                    orignalPullNodeId = Events.getOriginalReplica(events=events)(objectId)
                    operationId       = UUID.randomUUID().toString
                    _                 <- ctx.logger.info(s"REPLICA_PLACEMENT $objectId ${selectedNode.nodeId} $selectedServiceTime $operationId")
                    pullNodesIds      = List(orignalPullNodeId)
//                    _                 <- ctx.logger.debug(s"PULL_NODE_ID $pullNodesIds")
...
//                    pullArMap      = pullNodesX.map(x=>x.nodeId->x).toList.toMap
                    maybePullNodeX =  pullNodesXRaw.headOption
//                    maybePullNodeX = Events.balanceByReplica(currentState.downloadBalancerToken)(guid = objectId,arMap =pullArMap ,events=events,monitoringEvents = currentState.monitoringEvents)
                    _ <- maybePullNodeX match {
                      case Some(pullNodeX) => for {
//                        _           <- ctx.logger.debug(s"DATA_REPLICATION $objectId ${selectedNode.nodeId}")
                        _ <- IO.unit
                        dumbObject  = DumbObject(objectId = objectId,objectSize = replicateObjectSize)
//                        maybeActualNodes = distributionSchema.get(objectId)

//                        _ <- maybeActualNodes match {
//                          case Some(value) => ???
//                          case None => IO.unit
//                        }
//                        _           <- s
                        _           <- DataReplication.runEffects(
                          events=events,
                          dumbObject = dumbObject,
...
//          replicateObject  = availableNodesToReplicateMap.headOption
//          _  <- ctx.logger.debug(s"REPLICATE_OBJECT ${replicateObject.map(_._1)}")
...
//                    pullNodesIds            = List(Events.getOriginalReplica(events=events)(objectId))
//                    _ <- ctx.logger.debug(s"PULL_NODE_ID $pullNodesIds")
//                    pullNodesXRaw = Events.getNodesByIsd(pullNodesIds,events=events)
//                    pullNodesX    = NonEmptyList.fromListUnsafe(pullNodesXRaw)
//                    pullArMap     = pullNodesX.map(x=>x.nodeId->x).toList.toMap
